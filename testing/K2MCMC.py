...
for i, result in enumerate(sampler.sample(x0, iterations=nsteps, blobs0=blobs0)):
	...
	sys.stdout.write("\r{}[{}{}]{}".format('sampling EVEREST light curve... ', '#' * n, ' ' * (barwidth - n), ' (%s%%)' % str(100. * float(i) / nsteps)))
sys.stdout.write("\n")
...
for i, result in enumerate(sampler2.sample(pos, iterations=nsteps)):
	n = int((barwidth+1) * float(i) / nsteps)
	sys.stdout.write("\r{}[{}{}]{}".format('sampling folded light curve... ', '#' * n, ' ' * (barwidth - n), ' (%s%%)' % str(100. * float(i) / nsteps)))
sys.stdout.write("\n")
print 'sampling complete'
...
if annotate_plot == True:
	ant = AnchoredText('$T_0 = %s^{+%s}_{-%s}$ \n $Per = %s^{+%s}_{-%s}$ \n $R_p/R_* = %s^{+%s}_{-%s}$' % (
		BJD_offset+round(t0[0],4),round(t0[1],4),round(t0[2],4),
		round(per[0],4),round(per[1],4),round(per[2],4),
		round(rp_mcmc[0],4),round(rp_mcmc[1],4),round(rp_mcmc[2],4)), prop=dict(size=11), frameon=True, loc=3)
...
if save_plots == True:
	save_to = 'fig//K2_plots//%s' % targ_name
	if not os.path.exists(save_to):
		os.makedirs(save_to)
	fig_walkers_alpha.savefig('%s//%s_walkers1.png' % (save_to,epic), bbox_inches='tight')
	fig_corner_alpha.savefig('%s//%s_corner1.png' % (save_to,epic), bbox_inches='tight')
	final_fig.savefig('%s//%s_MCMCfit.png' % (save_to,epic), bbox_inches='tight')
	fig_corner_beta.savefig('%s//%s_corner2.png' % (save_to,epic), bbox_inches='tight')
	fig_walkers_beta.savefig('%s//%s_walkers2.png' % (save_to,epic), bbox_inches='tight')
	resid_fig.savefig('%s//%s_residuals.png' % (save_to,epic), bbox_inches='tight')
